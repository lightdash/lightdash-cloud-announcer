...
export const findGithubIssues = ({
  ...
  const searchQuerySchema = z.object({
    searchQueries: z.object({
      title: z
        .string()
        .describe("The title of the issue that should be searched for"),
      description: z
        .string()
        .describe("The description of the issue that should be searched for"),
      labels: z
        .array(z.string())
        .describe("The labels of the issue that should be searched for"),
      milestone: z
        .string()
        .describe("The milestone of the issue that should be searched for"),
    }),
  });
      ...
      const issues = await knex
        ...
        .from(function () {
          ...
          this.select("title", "description", "issue_url", {
            rank: knex.raw(`1 - (github_issues.embeddings <=> ?)`, [
              embeddings,
            ]),
          })
            .from("github_issues")
  ...
  const createGithubIssuesFromConversation = createWorkflow({
    ...
    .branch([
      ...
      [
        async ({ inputData }) => inputData.issues.length === 0,
        doNotSearchForIssues,
      ],
    ])
    .commit();
  ...
  const workflowRun = mastra
    .getWorkflow("createGithubIssuesFromConversation")
    .createRun();
